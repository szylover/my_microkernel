...
\begin{itemize}
  ...
  \item 了解红黑树后端的核心思想：5 条性质、旋转操作、$O(\log n)$ 保证（未来升级方向）
...
\begin{tikzpicture}[
  ...
  \node[box, fill=green!12] (sa)     at (-3.0,-4.0)
    {\texttt{vma\_sorted\_array.c}\\（本章实现）};
  \node[box, fill=green!8, dashed] (rb) at (3.0,-4.0)
    {\texttt{vma\_rbtree.c}\\（未来升级）};
  \draw[arr] (caller)--(api);
  \draw[arr] (api)--(disp);
  \draw[arr] (disp)--(sa);
  \draw[arr, dashed, gray] (disp)--(rb);
\end{tikzpicture}
...
\begin{exercise}
\textbf{实现红黑树后端 \texttt{vma\_rbtree.c}}

将排序数组后端升级为红黑树后端（满足 \texttt{vma\_ops\_t} 接口）：
\begin{enumerate}
  \item 使用静态数组 \texttt{node\_pool[256]}，每个节点包含 \texttt{vm\_area\_t}、颜色位和三个指针（parent/left/right）
  \item \texttt{add}：BST 插入 + 红黑修复（处理叔节点为红/黑、旋转）
  \item \texttt{find}：$O(\log n)$ 查找，与排序数组的二分查找复杂度相同
  \item \texttt{remove}：BST 删除 + 红黑修复（最复杂的部分，6 种情况）
\end{enumerate}
在 \texttt{kconfig.h} 中添加 \texttt{VMA\_BACKEND} 选项，\texttt{0}=sorted-array, \texttt{1}=rbtree。
用 \texttt{vma test} 验证通过，对比两种后端的代码量。
\end{exercise}
