...
\begin{tikzpicture}[node distance=0cm]
  ...
  \\draw[-{Stealth[length=4pt]}, thick] (big.south west) -- (left.north) node[midway, left, font=\\scriptsize] {返回给调用者};\n  \\draw[-{Stealth[length=4pt]}, thick] (big.south east) -- (right.north) node[midway, right, font=\\scriptsize] {放回 free\\_list[1]};", "oldString": "  \\draw[arr, -{Stealth}] (big.south west) -- (left.north) node[midway, left, font=\\scriptsize] {返回给调用者};\n  \\draw[arr, -{Stealth}] (big.south east) -- (right.north) node[midway, right, font=\\scriptsize] {放回 free\\_list[1]};
\end{tikzpicture}
