...
\begin{tikzpicture}[node distance=0cm]
  ...
  % 对应的物理页
  \foreach \i/\st in {0/used, 1/used, 2/free, 3/free, 4/used, 5/free, 6/free, 7/free} {
    \node[memcell, minimum width=0.9cm, minimum height=0.5cm,
          fill=\ifnum\i<2 red!15\else\ifnum\i=4 red!15\else green!15\fi\fi,
          font=\tiny] (p\i) at (\i*1.0, -1.2) {\st};
  }
...
\begin{tikzpicture}[node distance=0cm]
  ...
  \draw[arr, -{Stealth}] (big.south west) -- (left.north) node[midway, left, font=\scriptsize] {返回给调用者};
  \draw[arr, -{Stealth}] (big.south east) -- (right.north) node[midway, right, font=\scriptsize] {放回 free\_list[1]};
\end{tikzpicture}
