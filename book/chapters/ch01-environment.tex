...
«\tps{\$}» «\tin{sudo apt update}»
«\tps{\$}» «\tin{sudo apt install -y nasm gcc binutils grub-pc-bin}» \
    «\tin{xorriso qemu-system-x86 gcc-multilib gdb bear}»
\end{tcblisting}
...
\begin{tcblisting}{consolebox, title={Directory Structure}}
my_microkernel/
  src/
...
\begin{tcblisting}{consolebox}
«\tps{\$}» «\tin{cd src}»
«\tps{\$}» «\tin{make iso}»        # 编译内核 + 打包 ISO
«\tps{\$}» «\tin{make run}»        # QEMU 启动
«\tps{\$}» «\tin{make debug}»      # QEMU -S 等待 GDB 连接
«\tps{\$}» «\tin{make gdb}»        # 另一个终端启动 GDB
\end{tcblisting}
